...
def is_stage_2(df):
    ...
    if pd.to_numeric(df['close'].iloc[-1]) > pd.to_numeric(df['200dma'].iloc[-1]) and pd.to_numeric(df['200dma'].iloc[-1]) > pd.to_numeric(df['200dma'].iloc[-20]):
        return True
    return False
...
def is_stockbee_momentum(df):
    ...
    pct_change = ( pd.to_numeric(df['close'].iloc[-1]) - pd.to_numeric(df['close'].iloc[-2]) ) / pd.to_numeric(df['close'].iloc[-2])
