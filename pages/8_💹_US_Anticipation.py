...
st.set_page_config(page_title="📈 Anticipation-Tani US Screener", layout="wide")
st.title("📈 Anticipation-Tani US Screener (yfinance, EOD)")
...
run_btn = st.sidebar.button("Run Anticipation Screener")
...
if not run_btn:
    st.info("Press **Run Anticipation Screener** in the sidebar to start.")
    st.stop()
...
fscreener_df = pd.DataFrame(anticipation)

st.subheader("🔥 Anticipation Watchlist (Consolidating after strength):")
st.write(f"Found {len(fscreener_df)} Strong Matches Anticipation.")
...
if not fscreener_df.empty:
    ...
    st.download_button("Download strong matches CSV", data=csv2, file_name='Anticipation_Matches.csv', mime='text/csv')
